...
import fsExists from 'fs-exists-cached'
...
function validateFrontmatter<T extends z.ZodTypeAny>({
  ...
  try {
    ...
    `
    throw new Error(errMessage + (e as Error).message)
  }
}
...
export async function readOne<T extends z.ZodTypeAny>({
  ...
  const mdFilepath = path.join(contentDirectory, directory, `${slug}.md`)
  const mdxFilepath = path.join(contentDirectory, directory, `${slug}.mdx`)

  if (fsExists.sync(mdxFilepath)) {
    return read({
